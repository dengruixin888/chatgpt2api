...
import { Bot, ExternalLink, Globe2, LoaderCircle, Plus, Search, User } from "lucide-react";
...
function buildPromptWithHistory(prompt: string, turns: SearchTurn[]) {
  const history = turns
    ...
    .slice(-4)
    .map((turn, index) => [
      `第 ${index + 1} 轮用户问题：${turn.prompt}`,
      `第 ${index + 1} 轮回答摘要：${compactAnswer(turn.result?.answer || "")}`,
    ].join("\n"))
    .join("\n\n");
  ...
  return (
    <ReactMarkdown
      ...
      components={{
        ...
        h1: ({ className, ...props }) => <h1 className={cn("mt-6 mb-3 text-2xl font-semibold tracking-tight text-stone-950 first:mt-0 dark:text-stone-50", className)} {...props} />,
        h2: ({ className, ...props }) => <h2 className={cn("mt-6 mb-3 border-b border-stone-200 pb-2 text-xl font-semibold tracking-tight text-stone-950 first:mt-0 dark:border-white/10 dark:text-stone-50", className)} {...props} />,
        h3: ({ className, ...props }) => <h3 className={cn("mt-5 mb-2 inline-flex min-w-9 items-center justify-center rounded-full bg-stone-950 px-2.5 py-1 text-sm font-semibold text-white first:mt-0 dark:bg-white dark:text-stone-950", className)} {...props} />,
        ...
        blockquote: ({ className, ...props }) => <blockquote className={cn("my-4 rounded-r-2xl border-l-4 border-stone-300 bg-white/70 py-3 pr-4 pl-5 text-stone-700 dark:border-white/20 dark:bg-white/[0.04] dark:text-stone-300", className)} {...props} />,
        ...
        pre: ({ className, ...props }) => <pre className={cn("my-4 overflow-x-auto rounded-2xl border border-stone-200 bg-stone-950 p-4 text-sm text-stone-50 dark:border-white/10", className)} {...props} />,
        table: ({ className, ...props }) => <div className="my-4 overflow-x-auto rounded-2xl border border-stone-200 dark:border-white/10"><table className={cn("w-full border-collapse text-sm", className)} {...props} /></div>,
...
export function SearchPanel() {
  ...
  const runSearch = async () => {
    ...
    const sentPrompt = buildPromptWithHistory(value, turns);
    ...
    setLoading(true);
    setPrompt("");
    setTurns((items) => [...items, pendingTurn]);
    try {
      const result = await httpRequest<SearchResult>("/v1/search", { method: "POST", body: { prompt: sentPrompt } });
      setTurns((items) => items.map((turn) => turn.id === turnId ? { ...turn, result, elapsedMs: Date.now() - start, status: "success" } : turn));
    } catch (err) {
      setTurns((items) => items.map((turn) => turn.id === turnId ? { ...turn, error: err instanceof Error ? err.message : String(err), elapsedMs: Date.now() - start, status: "error" } : turn));
    } finally {
  ...
  return (
    <section className="relative mx-auto flex min-h-[calc(100vh-96px)] w-full max-w-6xl flex-col px-2 py-4 sm:px-4">
      ...
      <div className="relative mb-4 flex flex-wrap items-end justify-between gap-3 rounded-[28px] border border-white/70 bg-white/70 p-5 shadow-[0_24px_70px_rgba(87,72,55,0.12)] backdrop-blur dark:border-white/10 dark:bg-stone-950/55 dark:shadow-black/30">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-stone-500 dark:text-stone-400">
            支持追问和上下文延续。后续问题会带上最近几轮摘要，同时重新触发 ChatGPT 网页搜索。
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="relative flex min-h-0 flex-1 flex-col rounded-[28px] border border-white/70 bg-white/55 shadow-[0_18px_60px_rgba(87,72,55,0.10)] backdrop-blur dark:border-white/10 dark:bg-stone-950/40">
        <div className="flex-1 space-y-6 px-3 py-5 sm:px-5 lg:px-8">
          {turns.length === 0 ? (
            <div className="mx-auto flex min-h-[46vh] max-w-xl flex-col items-center justify-center text-center">
              ...
              <p className="mt-3 text-sm leading-7 text-stone-500 dark:text-stone-400">
                例如查资料、找最新状态、核实设定、继续追问上一轮结论。输入框默认留空，不再预置示例文本。
              </p>
            </div>
          ) : null}
            ...
            return (
              <div key={turn.id} className="space-y-3">
                <div className="ml-auto flex max-w-3xl items-start justify-end gap-3">
                  <div className="rounded-[22px] bg-stone-950 px-4 py-3 text-sm leading-7 text-white shadow-lg shadow-stone-900/10 dark:bg-white dark:text-stone-950">
                    {turn.prompt}
                  </div>
                    ...
                    {turn.result ? (
                      <>
                        <div className="text-[15px]">
                          <MarkdownResult content={normalizeMarkdown(turn.result.answer || "")} />
                        </div>
                        {sources.length ? (
                          <div className="mt-5 border-t border-stone-200 pt-4 dark:border-white/10">
                            ...
                            <div className="grid gap-2 sm:grid-cols-2">
                              {sources.slice(0, 8).map((source, index) => {
                                ...
                                return (
                                  <a key={`${url || index}`} href={url} target="_blank" rel="noreferrer" className="group flex gap-3 rounded-2xl border border-stone-200 bg-white/75 p-3 text-xs transition hover:-translate-y-0.5 hover:border-stone-300 hover:shadow-md dark:border-white/10 dark:bg-white/[0.03] dark:hover:bg-white/[0.07]">
                                    ...
                                    <span className="min-w-0">
                                      <span className="line-clamp-2 font-medium leading-5 text-stone-800 group-hover:text-stone-950 dark:text-stone-200 dark:group-hover:text-white">{source.title || sourceHost(url) || "source"}</span>
        ...
        >
          <div className="mx-auto flex max-w-4xl items-end gap-3 rounded-[24px] border border-stone-200 bg-white px-4 py-3 shadow-lg shadow-stone-900/5 dark:border-white/10 dark:bg-white/[0.04]">
            ...
            />
            <button type="submit" disabled={loading || !prompt.trim()} className="inline-flex size-10 shrink-0 items-center justify-center rounded-full bg-stone-950 text-white transition hover:bg-stone-800 disabled:cursor-not-allowed disabled:bg-stone-200 disabled:text-stone-400 dark:bg-white dark:text-stone-950 dark:hover:bg-stone-200 dark:disabled:bg-white/10 dark:disabled:text-stone-600">
